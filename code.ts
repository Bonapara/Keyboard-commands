...
function checkSpecialConditions(node: SceneNode, conditions: SpecialCondition[]): boolean {
  ...
  async function listFloatVariables(): Promise<string[]> {
    const localCollections = await figma.variables.getLocalVariableCollectionsAsync();
    const floatVariableNames: string[] = [];
    for (const collection of localCollections) {
      for (const variableId of collection.variableIds) {
        const variable = await figma.variables.getVariableByIdAsync(variableId);
        if (variable && variable.resolvedType === "FLOAT") {
          floatVariableNames.push(variable.name);
        }
      }
    }
    return floatVariableNames.sort();
  }  

  
  function buildCommandSummary(
    previousParts: string[],
    matchedCommand: Command & { name: CommandName },
    newValue: string
  ): string[] {
    const completeCommands = previousParts.map((part) => {
      const prevCmd = findCommand(part)[0];
      if (!prevCmd) return "Not Found";
      
      const { name, type } = prevCmd;
      ...
      if (type === 'commandWithValue' || type === 'optionalValueCommand') {
    ...
    if (matchedCommand.type === 'commandWithoutValue') {
      ...
      completeCommands.push(`${matchedCommand.name}:${newValue}`);
    }
  ...
  function setupInputHandler() {
    ...
    currentInputHandler = async ({ key, query, result }) => {
      ...
      parts.slice(0, -1).forEach(part => {
        ...
        if (matchedCommand) {
          if (matchedCommand.type === 'commandWithoutValue') {
            ...
            const hasHex = VALUE_FORMAT_REGEX.hex.exec(part);
            const hasNumber = VALUE_FORMAT_REGEX.number.exec(part);
            if (hasHex && 'valueFormat' in matchedCommand && matchedCommand.valueFormat === 'hex') {
      ...
      if (!query || query.endsWith(' ')) {
        result.setSuggestions(
          getCommandSuggestions(COMMANDS, '', undefined, true, previousCommands)
        );
        return;
      }
      ...
      const questionIndex = currentPart.indexOf('?');
      const isVariableFlow = 
      questionIndex !== -1 && 
      matchedCommand && 
      'valueFormat' in matchedCommand &&
      (matchedCommand.valueFormat === 'hex' || matchedCommand.valueFormat === 'number');
      
      if (matchedCommand) {
        if (isVariableFlow && 'valueFormat' in matchedCommand) {
          const typedVariableName = currentPart.slice(questionIndex + 1);
          
          let variableSuggestions: string[] = [];
          if (matchedCommand.valueFormat === 'hex') {
            variableSuggestions = await listColorVariables();
          } else if (matchedCommand.valueFormat === 'number') {
            variableSuggestions = await listFloatVariables();
          }    
          
          const filteredVars = variableSuggestions.filter(v =>
            v.toLowerCase().includes(typedVariableName.toLowerCase())
          );
          
          const defaultVar = filteredVars[0] ?? typedVariableName;
          const summaryValue = `?${defaultVar}`;
          const summary = buildCommandSummary(parts.slice(0, -1), matchedCommand, summaryValue);
          
          const suggestions = [
            summary.join(' | '),
            ...filteredVars.slice(1)
          ];
          
          if (filteredVars.length === 0) {
            suggestions.push(`No variable found for "${typedVariableName}"`);
          }
        ...
        const isValidValue =
        (matchedCommand.type === "commandWithValue" ||
          matchedCommand.type === "optionalValueCommand") &&
          'valueFormat' in matchedCommand && (
          ...
          // Your existing no-match handling logic
          ...
          if (allMatchingCommands.length === 0) {
            ...
            // Handle invalid commands for current selection
    ...
    // Whenever the selection changes, re-run setup
    ...
    async function executeCommand(cmd: string): Promise<void> {
      ...
      try {
        ...
        // Check for "?" usage (i.e., user typed a variable)
        const questionIndex = cmd.indexOf('?');
        const isVariableFlow =
          questionIndex !== -1 &&
          'valueFormat' in command &&
          command.valueFormat &&
          (command.valueFormat === 'hex' || command.valueFormat === 'number');
        
        if (isVariableFlow && 'valueFormat' in command && command.valueFormat) {
          // e.g. "TextColor?Text/Primary" or "Radius?BorderRadiusLarge"
          const variableName = cmd.slice(questionIndex + 1); // Everything after '?'
          await applyFigmaVariable(command, variableName);
        }
        else {
          // The original "no variable" path
          if (command.type === 'commandWithoutValue') {
            await processCommand(command.name);
          } else if ('valueFormat' in command && command.valueFormat) {
    ...
    async function applyFigmaVariable(command: Command, variableName: string) {
      const selection = figma.currentPage.selection;
